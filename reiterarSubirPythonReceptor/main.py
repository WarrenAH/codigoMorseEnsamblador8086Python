...
def descargarMostrarRespuesta():
    ...
    with open("mensajeRecibidoEmisor.txt", "wb") as archivo:
        metadata, descargarRespuesta = dbx.files_download(path="/bandejaMensajeEmisor/nuevoMensajeRecibido.txt")
        archivo.write(descargarRespuesta.content)

    with open('mensajeRecibidoEmisor.txt', 'r') as archivo:
        textoRespuesta = archivo.read().rstrip('\n')
        print("")
        print(textoRespuesta)
        print("")

    os.remove("mensajeRecibidoEmisor.txt")
...
def subirADropbox():
    ...
    archivoSubir = '/bandejaMensajeReceptor/nuevoMensajeRecibido.txt'
